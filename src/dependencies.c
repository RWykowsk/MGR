...
int compute_rhscand(int level)
{
  ...
  for (index = 0; index < noofcandidates[level]; index++) {
    ...
    while (element > 0) {
      ...
      //jezeli jest podzbior w tablicy podzbiorow levelu nizej o 1
      if (extHashTableEXISTS(C_sub[level-1],XA,&sub)) {
    	  //sklej X rhs z tym podzbiorem iloczyn logiczny
  ...
  for (index = 0; index < noofcandidates[level]; index++) {
    ...
    while (element > 0) {
      XA = X->name; extBitsetREMOVE(XA,element);
      //XA = X - element, potem sub to jest X - element jesli istnieje
      if (extHashTableEXISTS(C_sub[level-1],XA,&sub)) {
#ifndef USEDISK
#ifdef APPROXIMATE
        real_g3 = 0;
